    ...
    for i in range(WORD_COUNT):
        ...
        if "<" in originalScript[OS_IndexAt:]:
            ...
            if OS_nextIndex > tagStart and tagEnd >= OS_nextIndex:
                OS_nextIndex = originalScript.index(wordString, tagEnd) + len(
                    wordString
                )
        ...
        if "<" in nextDigest:
            ...
            strings[1] += (
                str.format("{0:.3f}", timeStart) + ",emotion," + str(emotion) + "\n"
            )
            prevPhoneme = "na"
        ...
        if "\n\n" in nextDigest:
            pararaph += 1
            image += (
                1  # The line of the script advances 2 lines whenever we hit a /n/n.
            )
            strings[0] += (
                str.format("{0:.3f}", timeStart) + ",paragraph," + str(pararaph) + "\n"
            )
            prevPhoneme = "na"
        ...
        if "\n" in nextDigest:
            image += 1
            strings[2] += (
                str.format("{0:.3f}", timeStart) + ",image," + str(image) + "\n"
            )
            prevPhoneme = "na"
            if not pickedPose:
                pickNewPose(
